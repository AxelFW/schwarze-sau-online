...
const midgameLeadCandidates = (cards, gs, player) => {
  if(!cards.length) return [];
  const mins = smallestCards(cards);
  if(mins.length === 1) return mins;
  const scores = mins.map(c => highUnplayedCount(gs, player, c.s));
  const maxScore = Math.max(...scores);
  return mins.filter((_, i) => scores[i] === maxScore);
};
