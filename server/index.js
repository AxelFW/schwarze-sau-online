...
function clearAdvanceTimer(roomCode) {
  const code = String(roomCode || "").trim().toUpperCase();
  if (!code) return;
  const entry = advanceTimers.get(code);
  if (!entry) return;
  clearTimeout(entry.timer || entry);
  advanceTimers.delete(code);
}

function automaticAdvanceKey(room) {
  if (!room || room.status !== "playing" || !room.game) return null;
  const game = room.game;
  const phase = game.phase;

  if (phase === "play") {
    const currentPlayer = game.gs?.currentPlayer;
    if (!Number.isInteger(currentPlayer)) return null;
    const publicState = publicRoom(room);
    const seat = publicState?.seats?.[currentPlayer];
    if (!seat?.botControlled) return null;

    const tricksPlayed = Number(game.gs?.tricksPlayed || 0);
    const trickLength = Array.isArray(game.gs?.trick) ? game.gs.trick.length : 0;
    const handLength = Array.isArray(game.gs?.hands?.[currentPlayer]) ? game.gs.hands[currentPlayer].length : 0;
    return "card:" + currentPlayer + ":" + tricksPlayed + ":" + trickLength + ":" + handLength;
  }

  if (phase === "quetsch") {
    const publicState = publicRoom(room);
    const selections = game.quetschSelections || [];
    const hasBotQuetschPending = publicState?.seats?.some((seat, i) => seat?.botControlled && !Array.isArray(selections[i]));
    const allReady = selections.length === 4 && selections.every((selection) => Array.isArray(selection) && selection.length === 3);
    if (!hasBotQuetschPending && !allReady) return null;
    return "quetsch:" + selections.map((selection) => Array.isArray(selection) ? "1" : "0").join("");
  }

  if (phase === "quetsch_review") {
    return "quetsch_review:" + Number(game.quetschReviewUntil || 0);
  }
...
function scheduleAdvance(roomCode, immediate = false, delayOverrideMs = null) {
  ...
  const key = automaticAdvanceKey(room);
  if (!key) {
    // Critical: if a human is now the current player, no old bot timer may survive.
    clearAdvanceTimer(code);
    return;
  }
  ...
  if (existing) clearAdvanceTimer(code);
  ...
  const scheduledKey = key;
  const timer = setTimeout(() => {
    const currentEntry = advanceTimers.get(code);
    if (currentEntry?.timer !== timer || currentEntry?.key !== scheduledKey) return;
    advanceTimers.delete(code);
    ...
    try {
      ...
      // If the room changed while the timer was pending, do not spend this old
      // timer on the new state. Schedule again so the new current bot gets a
      // fresh BOT_DELAY_MS from the moment it became current player.
      const liveKey = automaticAdvanceKey(room);
  ...
  timer.unref?.();
  advanceTimers.set(code, { timer, key });
}
